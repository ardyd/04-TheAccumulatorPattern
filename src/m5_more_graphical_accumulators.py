...
def run_test_draw_circles_from_rectangle():
    ...
    title = title + ' 8 blue in a row, 3 in a column; then 4 green in a row, 5 in a column'
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    inc = ((width / 2) + (height / 2))

    for k in range(m):
        circle = rg.Circle(rg.Point(center.x - k*height - inc, center.y), height/2)
        circle.fill_color = rectangle.fill_color
        circle.attach_to(window)

    for k in range(n):
        circle = rg.Circle(rg.Point(center.x, center.y - k*width - inc), width/2)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    lc = rectangle1.get_lower_left_corner()

    # get change in distance between x,y of each line
    xc = center1.x - lc.x
    yc = center1.y - lc.y
    ...
    for k in range(n):
        line = rg.Line(rg.Point(center1.x - xc*k, center1.y - yc*k), rg.Point(center2.x - xc*k, center2.y - yc*k))
        line.thickness = 5
        if k % 2 == 0:
